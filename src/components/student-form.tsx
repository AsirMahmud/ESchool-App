...
const studentFormSchema = z.object({
  ...
  department: z.number().min(1).optional(),
...
interface StudentFormProps {
  ...
  student?: any // Student data for editing
  onSuccess?: () => void
}

export function StudentForm({ open, onOpenChange, student, onSuccess }: StudentFormProps) {
  const [selectedLevel, setSelectedLevel] = useState<number | undefined>(student?.level || undefined)
  const [selectedSection, setSelectedSection] = useState<number | undefined>(student?.section || undefined)
  ...
  const filteredSections = sections?.filter(section => section.level === selectedLevel && selectedLevel > 0) || []
  ...
  useEffect(() => {
    ...
    if (selectedLevel !== currentLevel) {
      ...
      form.setValue('section', undefined)
    }
  }, [selectedLevel, form])
  ...
  useEffect(() => {
    if (student) {
      setSelectedLevel(student.level || undefined)
      setSelectedSection(student.section || undefined)
    }
  }, [student])
